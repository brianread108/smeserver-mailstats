...
# bjr - 07Aug17 - Fix up log items tags so that only relevant items shown on detail web page - TBD 
#
#
#############################################################################
#
...
# 3. Delete earlier logData and LogDataCounts records - up to parameter daysKeepLogData
# 4. Generate unique html page - keep up to daysKeepHTMLPage
...
#@emails_per_count_id = ({mailid=>1,countid=>1},
														#{mailid=>2,countid=>1},
														#{mailid=>2,countid=>3},
														#{mailid=>3,countid=>4});
...
#
#Load up the emails currently stored for DMARC reporting - so that we cna spot the reports being sent.
...
my $recip_caption="";
my $virus_caption="";
my $virus_table;@{$virus_table}={};
my $qpsmtpd_caption="";
my $qpsmtpd_table;
my $geoip_caption="";
my $geoip_table;@{$geoip_table}={};
my $junkmail_caption="";
my $junkmail_table;@{$junkmail_table}={};
my $blacklistsettings_caption="";
my $blacklistsettings_table;@{$blacklistsettings_table}={};
my $blacklistuse_caption="";
my $blacklistuse_table;@{$blacklistuse_table}={};
my $emails_caption="";
my $emails_table;
my $bottombit = "";
...
my $makeHTMLemail = "no";
if ($cdb->get('mailstats')){$makeHTMLemail = $cdb->get('mailstats')->prop('HTMLEmail') || "no"} 
my $makeHTMLpage = "no";
...
#print STDERR $makeHTMLemail,$makeHTMLpage;


# Init the hashes
my $nhour = floor( $start / 3600 );
my $ncateg;
my $count_id_index = 0;  #Unique count_id - might need to be setup a unique start.
...
LINE: while (<>) {
    ...
    #dbg("REST:".$log);
		...
		if ($savedata) {
			...
			#print STDERR $_."\n".$CurrentLine."\n".$1."\n".$2;
			#exit;
			#$1 = tai64nlocal time and Date, $2 = MailId
			...
			if ($l != 0){
				if (defined($2)){		#print STDERR "Undefined \$2:".$_.":".$count.":".$l;exit}
					...
					$LogLines{$CurrentLogId.":".$Unique.":".$Sequence} = $_;
				}
			}
...
# and adjust for recipient field if not set-up by denying plugin - extract from deny msg
   ...
   # then categorise the result
	...
	if (exists $log_items[5]) {
		...
		if ($log_items[5] eq 'check_earlytalker') {$MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'check_relay') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'check_norelay') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'require_resolvable_fromhost') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'check_basicheaders') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'rhsbl') { $RBLcount++;$counts{$abshour}{$CATRBLDNS}++;push_count_ids($CurrentMailId,$abshour,$CATRBLDNS);mark_domain_rejected($proc);$blacklistURL{get_domain($log_items[7])}++;}

		elsif ($log_items[5] eq 'dnsbl') { $RBLcount++;$counts{$abshour}{$CATRBLDNS}++;push_count_ids($CurrentMailId,$abshour,$CATRBLDNS);mark_domain_rejected($proc);$blacklistURL{get_domain($log_items[7])}++;}

		elsif ($log_items[5] eq 'check_badmailfrom') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'check_badrcptto_patterns') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'check_badrcptto') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'check_spamhelo') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'check_goodrcptto extn') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'rcpt_ok') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'pattern_filter') { $PatternFilterCount++;$counts{$abshour}{$CATEXECUT}++;push_count_ids($CurrentMailId,$abshour,$CATEXECUT);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'virus::pattern_filter') { $PatternFilterCount++;$counts{$abshour}{$CATEXECUT}++;push_count_ids($CurrentMailId,$abshour,$CATEXECUT);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'check_goodrcptto') {$MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'check_smtp_forward') {$MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'count_unrecognized_commands') {$MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'check_badcountries') {$MiscDenyCount++;$counts{$abshour}{$CATBADCOUNTRIES}++;push_count_ids($CurrentMailId,$abshour,$CATBADCOUNTRIES);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'tnef2mime') { } #Not expecting this one.

		elsif ($log_items[5] eq 'spamassassin') { 
						$above15++;$counts{$abshour}{$CATSPAMDEL}++;
		...
		#            if ($log_items[8] =~ "Yes, hits=(.*) required=([0-9\.]+)") 
						...
						if ($log_items[8] =~ ".*score=([+-]?\\d+\.?\\d*).* required=([0-9\.]+)") {
							...
							{
								if ($score < $SATagLevel) { $hamcount++;$counts{$abshour}{$CATHAM}++;push_count_ids($CurrentMailId,$abshour,$CATHAM);$hamavg += $score;}
								else {$spamcount++;$counts{$abshour}{$CATSPAM}++;push_count_ids($CurrentMailId,$abshour,$CATSPAM);$spamavg += $score;$result= "spam";}
							} else {
		...
		elsif ($log_items[5] eq 'tls') {$MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'auth::auth_cvm_unix_local') {$MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'earlytalker') {$MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'uribl') {$RBLcount++;$counts{$abshour}{$CATRBLDNS}++;push_count_ids($CurrentMailId,$abshour,$CATRBLDNS);mark_domain_rejected($proc);$blacklistURL{get_domain($log_items[7])}++;}
		 ...
		 if ($log_items[7] =~ m/(karma)/) {
			 $MiscDenyCount++;$counts{$abshour}{$CATKARMA}++;push_count_ids($CurrentMailId,$abshour,$CATKARMA);mark_domain_rejected($proc);}
		 elsif ($log_items[7] =~ m/(dnsbl)/){
			 $RBLcount++;$counts{$abshour}{$CATRBLDNS}++;push_count_ids($CurrentMailId,$abshour,$CATRBLDNS);mark_domain_rejected($proc);$blacklistURL{get_domain($log_items[7])}++;}
		 elsif ($log_items[7] =~ m/(helo)/){
				$MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}
		 else {
			 #Unidentified Naughty rejection
			 $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);$unrecog_plugin{$log_items[5]."-".$log_items[7]}++;}
		}
		elsif ($log_items[5] eq 'resolvable_fromhost') {$MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'loadcheck') {$MiscDenyCount++;$counts{$abshour}{$CATLOAD}++;push_count_ids($CurrentMailId,$abshour,$CATLOAD);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'karma') {$MiscDenyCount++;$counts{$abshour}{$CATKARMA}++;push_count_ids($CurrentMailId,$abshour,$CATKARMA);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'dmarc') {$MiscDenyCount++;$counts{$abshour}{$CATDMARC}++;push_count_ids($CurrentMailId,$abshour,$CATDMARC);mark_domain_rejected($proc)}

		elsif ($log_items[5] eq 'relay') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'headers') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'mailfrom') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'badrcptto') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'helo') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'check_smtp_forward') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

		elsif ($log_items[5] eq 'sender_permitted_from') { $MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);}

	 #Treat it as Unconf if not recognised
	 else {$MiscDenyCount++;$counts{$abshour}{$CATNONCONF}++;push_count_ids($CurrentMailId,$abshour,$CATNONCONF);mark_domain_rejected($proc);$unrecog_plugin{$log_items[5]}++;}
 } #Log[5] exists
...
if ( !$disabled ) {
    ...
    print "*SMEServer daily email statistics from $hostname - ".$todaydate."*\n";
    print "----------------------------------------------------------------------------------", "\n\n";
    my $version = "$0 Version : $opt{'version'}";
    push(@{$topbit},{f1=>"$version\n"});
    push(@{$topbit},{f1=>"Period Beginning : ".strftime( "%c", localtime($start) )."\n"});
#    push(@{$topbit},{f1=>print("Period Beginning : ", strftime( "%c", localtime($start) ), "\n")});
    push(@{$topbit},{f1=>"Period Ending    : ".strftime( "%c", localtime($end) )."\n"});
    push(@{$topbit},{f1=>"Clam Version/DB Count/Last DB update: ".`freshclam -V`});
    push(@{$topbit},{f1=>"SpamAssassin Version : ".`spamassassin -V`});
    push(@{$topbit},{f1=>sprintf("Tag level: %3d; Reject level: %3d $warnnoreject\n", $SATagLevel,$SARejectLevel)});
    if ($HighLogLevel) {
      push(@{$topbit},{f1=>sprintf("*Loglevel is set to: ".$LogLevel. " - you only need it set to 6\n")});
      push(@{$topbit},{f1=>sprintf("\tYou can set it this way:\n")});
      push(@{$topbit},{f1=>sprintf("\tconfig setprop qpsmtpd LogLevel 6\n")});
      push(@{$topbit},{f1=>sprintf("\tsignal-event email-update\n")});
      push(@{$topbit},{f1=>sprintf("\tsv t /var/service/qpsmtpd\n")});
    }
    push(@{$topbit},{f1=>sprintf("Reporting Period : %.2f hrs\n", $hrsinperiod)});
    push(@{$topbit},{f1=>sprintf("All SMTP connections accepted:%8d          \n", $totalexamined)});
    push(@{$topbit},{f1=>sprintf("Emails per hour              : %8.1f/hr\n", $emailperhour|| 0)});
    push(@{$topbit},{f1=>sprintf("Average spam score (accepted): %11.2f\n", $spamavg       || 0)});
    push(@{$topbit},{f1=>sprintf("Average spam score (rejected): %11.2f\n", $rejectspamavg || 0)});
    push(@{$topbit},{f1=>sprintf("Average ham score            : %11.2f\n", $hamavg        || 0)});
    push(@{$topbit},{f1=>sprintf("Number of DMARC reporting emails sent:\t%11d (not shown on table)\n", $DMARCSendCount      || 0)});
    if ($hamcount != 0){push(@{$topbit},{f1=>sprintf("Number of emails approved through DMARC:\t%11d (%3d%% of Ham count)\n", $DMARCOkCount||0,$DMARCOkCount*100/$hamcount||0)});}
    ...
    while ( $hour <= $end / 3600 ) {
			...
			if ($hour == floor( $start / 3600 )){
					...
					while ( $ncateg < @categs) {
						if ($finaldisplay[$ncateg]){
								...
								$Line2 .= " ";
								$title = sprintf('%'.($colwidth[$ncateg]-1).'s',$categs[$ncateg]);
								$Titles .= $title."|";
			...
			if ($hour < $end / 3600){
				while ( $ncateg < @categs) {
					if ($finaldisplay[$ncateg]){
							if ($ncateg == 0) {
								$value = strftime( "%F, %H", localtime( $hour * 3600 ) )." "
							} elsif ($ncateg == @categs-1) {
								#percentages in last column
								$value = sprintf('%'.($colwidth[$ncateg]-2).'.1f',$counts{$hour}{$categs[$ncateg]})."%";
							} else {
								#body numbers
									 $value = sprintf('%'.($colwidth[$ncateg]-1).'d',$counts{$hour}{$categs[$ncateg]})." ";
							 }
							 if (($ncateg == @categs-1)){$value=$value."\n";} #&& ($hour == floor($end / 3600)-1)
							 push(@{$stats_table_cols},{hour=>$hour,categ=>$categs[$ncateg],col=>$value,id=>$count_id{$hour}{$categs[$ncateg]}});
							 $Values .= $value;
					}
					$ncateg++
				}
					...
					while ( $ncateg < @categs) {
						if ($finaldisplay[$ncateg]){
								if ($ncateg == 0) {
									...
									if ($ncateg==@categs-1){
										$total = sprintf('%'.$colwidth[$ncateg].'.1f',$counts{$GRANDTOTAL}{$categs[$ncateg]}).'%';
									} else {
											$total = sprintf('%'.$colwidth[$ncateg].'d',$counts{$GRANDTOTAL}{$categs[$ncateg]});
									}
									$Totals .= $total;
									push(@{$stats_table_cols_totals},{col=>$total,id=>$count_id{$GRANDTOTAL}{$categs[$ncateg]}});   		
								}
						}
						$ncateg++
					}
					...
					while ( $ncateg < @categs) {
						...
						if ($finaldisplay[$ncateg]){
							if ($ncateg == 0) {
								...
								$Percent .= $percent;
								push(@{$stats_table_cols_percent},{col=>$percent});   		
							}
    ...
    if ($localAccepttotal>0) {
      push(@{$stats_footnote},{f1=>"*Fetchml* means connections from Fetchmail delivering email\n"});
    }
    push(@{$stats_footnote},{f1=>"*Local* means connections from workstations on local LAN.\n\n"});
    push(@{$stats_footnote},{f1=>"*Non\.Conf\.* means sending mailserver did not conform to correct protocol"});
    push(@{$stats_footnote},{f1=>"  or email was to non existant address.\n\n"});
   if ($finaldisplay[$KarmaCateg]){
			push(@{$stats_footnote},{f1=>"*Karma* means email was rejected based on the mailserver's previous activities.\n\n"});
   }
   if ($finaldisplay[$BadCountryCateg] or $total_countries > 0){
        $BadCountries  = $cdb->get('qpsmtpd')->prop('BadCountries') || "*none*";
				push(@{$stats_footnote},{f1=>"*Geoip\.*:Bad Countries mask is:".$BadCountries."\n\n"});
   }
   if (scalar keys %unrecog_plugin > 0){
		 #Show unrecog plugins found
		 push(@{$stats_footnote},{f1=>"*Unrecognised plugins found - categorised as Non-Conf\n"});
		 foreach my $unrec (keys %unrecog_plugin){
			 push(@{$stats_footnote},{f1=>"\t$unrec\t($unrecog_plugin{$unrec})\n"});
			}
			push(@{$stats_footnote},{f1=>"\n"});
		}
    if ($QueryNoLogTerse) {
      push(@{$stats_footnote},{f1=>"* - as no records where found, it looks as though you may not have the *logterse* \nplugin running as part of qpsmtpd \n\n"});
#      print " to enable it follow the instructions at .............................\n"});
    }
    if ( !$RHSenabled or !$DNSenabled ) {
        # comment about RBL not set
        push(@{$stats_footnote},{f1=>"* - This means that one or more of the possible spam black listing services\n    that are available have not been enabled.\n"});
        ...
        push(@{$stats_footnote},{f1=>" To enable these you can use the following commands:\n"});
        if ( !$RHSenabled ) {
            push(@{$stats_footnote},{f1=>" config setprop qpsmtpd RHSBL enabled\n"});
        }
        if ( !$DNSenabled ) {
            push(@{$stats_footnote},{f1=>" config setprop qpsmtpd DNSBL enabled\n"});
        }
        # there so much templates to expand... (PS)
        push(@{$stats_footnote},{f1=>" Followed by:\n signal-event email-update and\n sv t /var/service/qpsmtpd\n\n"});
    }
	...
	if ($makeHTMLemail eq "yes" or $makeHTMLemail eq "both" or $makeHTMLpage eq "yes"){
		#print STDERR $dateid.$servername;
		require HTML::Template;
		# see front Comment about installing this....
		my $template = HTML::Template->new(filename => 'mailstats.tmpl',loop_context_vars => 1,die_on_bad_params => 0,global_vars => 1);
		...
		if ($makeHTMLpage eq "yes"){
			...
			my $filename = $htmlpagepath."mailstats.html";
		...
		if ($makeHTMLemail eq "yes" or $makeHTMLemail eq "both"){
			# create html email
			if ( $opt{'mail'} and !$disabled ) {
				#print STDERR "making html email";
				#Add in styles html email
				local $/;
				my $htmlstylefile = $htmlpagepath."mailstats.css";
				open my $fh, '<', $htmlstylefile or die "can't open $htmlstylefile: $!";
				my $htmlstyle =  <$fh>;
				...
				if ($makeHTMLemail eq "both"){
					...
					print SENDMAIL ' boundary="------------1916B385DAEB611B3336A1A0"'."\n";
					print SENDMAIL "This is a multi-part message in MIME format.";
					print SENDMAIL "--------------1916B385DAEB611B3336A1A0\n";
					print SENDMAIL "Content-Type: text/plain; charset=utf-8; format=flowed;\n";
...
sub show_recip_usage {
        ...
        )
    {
        ...
        push(@{$recip_cols},{col=>sprintf("%6.2f%%",$ac*100/$to)});
...

{
                ...
                keys %found_viruses)
    {
			...
			if (index($virus,"Sanesecurity") !=-1 or index($virus,"UNOFFICIAL") !=-1){
				push(@{$virus_cols},{col=>"Rejected\t",col=>"$found_viruses{$virus}\thttp://sane.mxuptime.com/s.aspx?id=$virus\n"});
			} else {
					push(@{$virus_cols},{col=>"Rejected\t",col=>"$found_viruses{$virus}\t$virus\n"});
			}
...

{
    ...
    {  
      my $qpsmtpd_cols; 
			...
			push(@{$qpsmtpd_cols},{col=>sprintf('%4.1f%%',$found_qpcodes{$qpcode}*100/$totalexamined)});
...

{
    ...
    print($line);
    my %lists = (RHSBL=>"RBLList",URIBL=>"UBLList",SBLList=>"SBLList");
    foreach my $list (keys %lists) {
			if ($cdb->get('qpsmtpd')->prop("$list") eq "enabled") {
				...
				push(@{$blacklistsettings_cols},{col=>$cdb->get('qpsmtpd')->prop($lists{$list})});
				foreach my $str (@$blacklistsettings_cols){ 
					print $$str{"col"}."\t";
				}
	    	push(@{$blacklistsettings_table},{cols=>$blacklistsettings_cols});			
				print "\n";
			}
		}
    #if ($cdb->get('qpsmtpd')->prop("RHSBL") eq "enabled") {
			#print "RBLLIST:".$cdb->get('qpsmtpd')->prop("RBLList")."\n";}
    #if ($cdb->get('qpsmtpd')->prop("URIBL") eq "enabled") {
			#print "UBLLIST:".$cdb->get('qpsmtpd')->prop("UBLList")."\n";}
    #if (!$cdb->get('qpsmtpd')->prop("SBLList") eq "") {
			#print "SBLLIST:".$cdb->get('qpsmtpd')->prop("SBLList")."\n";}
    print($line);
                ...
                keys %blacklistURL)
    {
				...
				push(@{$blacklistuse_cols},{col=>sprintf('%3u',$blacklistURL{$blcode})});
...
sub List_Junkmail {
    ...
    if ( $i > 0 ) {
			...
			print("\n".$line);
			foreach my $thisuser (sort { $junkcount{$b} <=> $junkcount{$a} }
					keys %junkcount)
			{
				my $junkmail_cols; 
				push(@{$junkmail_cols},{col=>sprintf("%d",$junkcount{$thisuser})});
...

{
    ...
    {
			my $emails_cols; 
			push(@{$emails_cols},{col=> sprintf('%3u',$usercounts{$user}{"queued"})."\t"});
			push(@{$emails_cols},{col=>sprintf('%3u',$usercounts{$user}{"rejected"})."\t\t"});
			push(@{$emails_cols},{col=>sprintf('%3u',$usercounts{$user}{"spam"})."\t\t"});
			my $usernomailto = $user; $usernomailto =~ s/(\<|\>)//g;
    ...
    print($line);
    my $emails_footcols; 
		push(@{$emails_footcols},{col=> sprintf('%3u',$totalqueued)."\t"});
		push(@{$emails_footcols},{col=>sprintf('%3u',$totalrejected)."\t\t"});
...
#
{
    ...
    if ($total_countries > 0) {
									...
									keys %found_countries)
			{
				 if ($total_countries>0) {
					 ...
					 if ( $percent >= $percentthreshold ) {
						...
						push(@{$geoip_cols},{col=>sprintf( '%4.1f', $percent )."\t"});
						push(@{$geoip_cols},{col=>"\t$found_countries{$country}"});
			...
			if ($showtotals){
				...
				push(@{$geoip_cols},{col=>sprintf("%4.1f",$totalpercent)});
...
#            .$counts{$nhour}{$categs[$ncateg]}.")\n");
	...
	if ($makeHTMLemail eq 'yes' or $makeHTMLemail eq 'both' or $makeHTMLpage eq 'yes'){
		$dbh->do("DELETE from LogData WHERE dateid = ".$dateid." AND servername='".$servername."'");
		foreach my $logid (keys %LogLines){
			...
			my $sql = "INSERT INTO LogData (Log64n,MailID,Sequence,LogStr,dateid,servername) VALUES ('";
			$sql .= $Log64n."','".$LogMailId."','".$LogSeq."',".$LogLine.",".$dateid.",'".$servername."')";
			$dbh->do($sql) or die("Write to LogData $sql");
			$reccount++;
		}
		# and the Save the link between the counts and the loglines
		$dbh->do("DELETE from LoglinesInCount WHERE dateid = ".$dateid." AND servername='".$servername."'");
		foreach my $i ( 0 ..$#emails_per_count_id){
			my $Mailid = $emails_per_count_id[$i]{mailid};
			my $countid = $emails_per_count_id[$i]{countid};
			my $sql = "INSERT INTO LoglinesInCount (MailId,Count_id,dateid,servername) VALUES ('";
			$sql .= $Mailid."','".$countid."',".$dateid.",'".$servername."')";
	...
	#print STDERR "Saved $reccount records in $telapsed sec.\n";
}
...
sub push_count_ids
# 		push_count_id($CurrentMailId,$abshour,$CATLOCAL);
{   
	my $CurrentMailId = shift;
	my $abshour = shift;
	my $CATEGORY = shift;
	push(@emails_per_count_id,{mailid=>$CurrentMailId,countid=>$count_id{$abshour}{$CATEGORY}});
	push(@emails_per_count_id,{mailid=>$CurrentMailId,countid=>$count_id{$GRANDTOTAL}{$CATEGORY}});
	push(@emails_per_count_id,{mailid=>$CurrentMailId,countid=>$count_id{$abshour}{$categs[@categs-2]}});
	push(@emails_per_count_id,{mailid=>$CurrentMailId,countid=>$count_id{$GRANDTOTAL}{$categs[@categs-2]}});}
